// Pixel DigDug - screen flow, on-screen + keyboard input, RAF, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-digdug:save';
...
function refreshDir() {
  if (!state) return;
  // Pick the most recent direction button still held.
  let last = null;
  for (const k of ['left','right','up','down']) if (touchHeld[k] != null) last = k;
  state.inputDir = last;
}
  ...
  if (screen === 'play') {
    ...
    for (const key of Object.keys(rs)) {
      if (hit(p, rs[key])) {
        ...
        if (key === 'pump') setPump(state, true);
        else refreshDir();
        return;
      }
    }
...
canvas.addEventListener('pointerup', (e) => {
  for (const key of Object.keys(touchHeld)) {
    if (touchHeld[key] === e.pointerId) {
      ...
      if (key === 'pump') setPump(state, false);
      else refreshDir();
    }
  }
});
...
window.addEventListener('keydown', (e) => {
  ...
  if      (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A')  { touchHeld['left']  = -1; refreshDir(); }
  else if (e.key === 'ArrowRight'|| e.key === 'd' || e.key === 'D')  { touchHeld['right'] = -1; refreshDir(); }
  else if (e.key === 'ArrowUp'   || e.key === 'w' || e.key === 'W')  { touchHeld['up']    = -1; refreshDir(); }
  else if (e.key === 'ArrowDown' || e.key === 's' || e.key === 'S')  { touchHeld['down']  = -1; refreshDir(); }
  else if (e.key === ' ' || e.key === 'z' || e.key === 'Z')          { setPump(state, true); }
});
window.addEventListener('keyup', (e) => {
  if (!state) return;
  if      (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A')  { delete touchHeld['left'];  refreshDir(); }
  else if (e.key === 'ArrowRight'|| e.key === 'd' || e.key === 'D')  { delete touchHeld['right']; refreshDir(); }
  else if (e.key === 'ArrowUp'   || e.key === 'w' || e.key === 'W')  { delete touchHeld['up'];    refreshDir(); }
  else if (e.key === 'ArrowDown' || e.key === 's' || e.key === 'S')  { delete touchHeld['down'];  refreshDir(); }
  else if (e.key === ' ' || e.key === 'z' || e.key === 'Z')          { setPump(state, false); }
});
...
function paintTitle() {
  ...
  // Inline demo: tiny board patch.
  const demo = buildGame(0);
  drawBoard(ctx, demo);
  drawPlayer(ctx, demo.player);
  drawEnemies(ctx, demo);
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillText(LEVELS[i].enemies + ' foes · ' + LEVELS[i].rocks + ' rocks', r.x + 8, r.y + 34);
    ctx.textAlign = 'right';
    if (cleared) {
      ctx.fillStyle = '#ffe04a';
...
function paintPlay() {
  ...
  drawControls(ctx, state.inputDir, state.inputPump);
  drawButton(t(lang, 'backToMenu'), backRect(), true);
}
