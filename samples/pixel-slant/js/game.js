// Pixel Slant - screen flow, tap input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-slant:save';
...
function startLevel(i) {
  ...
  cells = new Array(puzzle.C * puzzle.C).fill(D_NONE);
...
function tapCell(cell) {
  cells[cell] = (cells[cell] + 1) % 3;          // blank -> \ -> / -> blank
  if (isSolved(puzzle.C, puzzle.clues, cells)) {
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillStyle = can ? PALETTE.panel : '#1a1c24';
...
function paintPlay() {
  drawBackdrop(ctx);
  drawBoard(ctx, puzzle, cells);
  drawHud(ctx, lang, puzzle, cells, elapsed);
...
function drawButton(label, r, dim) {
  ctx.fillStyle = dim ? '#2a2e3c' : '#54c9c0';
  ctx.fillRect(r.x, r.y, r.w, r.h);
  ctx.fillStyle = dim ? '#3a3f50' : '#86e6df';
  ctx.fillRect(r.x, r.y, r.w, 2);
  ctx.fillStyle = dim ? '#f3f1e6' : '#10131a';
  ctx.font = 'bold 12px monospace';
  ctx.textAlign = 'center';
