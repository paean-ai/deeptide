// Pixel Copter - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-copter:save';
...
let result = null;
let lastTime = 0;
const held = new Set();                    // pointerIds currently pressed
...
function bestOf(i) { return save.best[String(i)] | 0; }
...
function loop(now) {
  ...
  if (screen === 'play' && state) {
    setInput(state, held.size > 0);
    tick(state, dt);
    if (state.over) {
      const dist = state.distance | 0;
      result = { levelIndex: state.levelIndex, distance: dist };
      // Pass threshold for "cleared" = a chunky run.
      const target = 1500 + state.levelIndex * 500;
      const cleared = dist >= target;
      if (cleared) {
        if (!save.cleared.includes(state.levelIndex)) save.cleared.push(state.levelIndex);
      }
      const key = String(state.levelIndex);
      if (!save.best[key] || save.best[key] < dist) save.best[key] = dist;
      persist();
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (screen === 'result') {
    ...
    if (result.cleared && result.levelIndex + 1 < LEVEL_COUNT && hit(p, nextRect())) {
...
canvas.addEventListener('pointerup', (e) => { held.delete(e.pointerId); });
canvas.addEventListener('pointercancel', (e) => { held.delete(e.pointerId); });

// Keyboard fallback.
const keysDown = new Set();
window.addEventListener('keydown', (e) => {
  if (screen !== 'play') return;
  if (e.key === ' ' || e.key === 'ArrowUp' || e.key === 'w') { keysDown.add(e.key); held.add('key'); }
});
window.addEventListener('keyup', (e) => {
  keysDown.delete(e.key);
  if (!keysDown.size) held.delete('key');
});
...
function paintTitle() {
  ...
  for (let i = 0; i < 40; i++) ctx.fillRect((i * 41 + 7) % CANVAS_W, (i * 73 + 17) % CANVAS_H, 1, 1);
  ...
  ctx.fillRect(170, 200, 20, 10);
  ctx.fillStyle = PALETTE.glass;
  ctx.fillRect(185, 202, 4, 4);
  ctx.fillStyle = PALETTE.rotor;
  ctx.fillRect(165, 196, 30, 1);
  drawButton(t(lang, 'start'), titleStartRect());
  drawButton(lang === 'en' ? '中文' : 'English', langRect(), true);
  ctx.fillStyle = '#bfc7e6';
  ctx.font = '10px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(t(lang, 'rulesTxt1'), CANVAS_W / 2, 350);
  ctx.fillText(t(lang, 'rulesTxt2'), CANVAS_W / 2, 366);
  ctx.fillText(t(lang, 'rulesTxt3'), CANVAS_W / 2, 382);
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.font = '10px monospace';
    ctx.fillText('speed ' + LEVELS[i].speed + ' · gap ' + LEVELS[i].gap + '→' + LEVELS[i].gapMin, r.x + 8, r.y + 36);
    ctx.textAlign = 'right';
    const b = bestOf(i);
    if (b) ctx.fillText((cleared ? t(lang, 'cleared') + ' · ' : '') + 'best ' + b, r.x + r.w - 8, r.y + 36);
...
function paintPlay() {
  drawScene(ctx, state, CANVAS_W, CANVAS_H, lang, bestOf(state.levelIndex));
  if (!state.started) {
    ctx.fillStyle = 'rgba(247, 230, 154, 0.9)';
    ctx.font = 'bold 14px monospace';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(t(lang, 'hold'), CANVAS_W / 2, 430);
...
function paintResult() {
  ...
  ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);
  ctx.fillStyle = result.cleared ? '#f7e69a' : '#e8554f';
  ctx.font = 'bold 24px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(t(lang, 'win'), CANVAS_W / 2, 160);
  ctx.fillStyle = '#f8f5e8';
  ctx.font = '14px monospace';
  ctx.fillText(t(lang, 'distance') + ': ' + result.distance + ' / ' + result.target, CANVAS_W / 2, 200);
  ctx.fillText(t(lang, 'best') + ': ' + bestOf(result.levelIndex), CANVAS_W / 2, 222);
  drawButton(t(lang, 'retry'), retryRect());
  if (result.cleared && result.levelIndex + 1 < LEVEL_COUNT) drawButton(t(lang, 'next'), nextRect());
