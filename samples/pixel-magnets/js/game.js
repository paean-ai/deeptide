// Pixel Magnets - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-magnets:save';
...
function startLevel(i) {
  ...
  if (!puzzle) return;
  marks = new Array(puzzle.W * puzzle.H).fill(UNKNOWN);
  selected = null;
...
function cycleCell(i) {
  if (i < 0 || i >= puzzle.W * puzzle.H) return;
  // UNKNOWN -> POS -> NEG -> NEUTRAL -> UNKNOWN
  const v = marks[i];
  if (v === UNKNOWN)      marks[i] = POS;
  else if (v === POS)     marks[i] = NEG;
  else if (v === NEG)     marks[i] = NEUTRAL;
  else                    marks[i] = UNKNOWN;
  if (isSolved(puzzle.W, puzzle.H, puzzle.dominoes, puzzle.rc, marks)) {
...
function paintTitle() {
  ...
  // Demo: a single magnet with + and -.
  ctx.fillStyle = PALETTE.cell;
  ctx.fillRect(160, 200, 40, 80);
  ctx.fillStyle = PALETTE.pos;
  ctx.fillRect(164, 204, 32, 36);
  ctx.fillStyle = PALETTE.neg;
  ctx.fillRect(164, 244, 32, 32);
  ctx.fillStyle = '#fff';
  ctx.fillRect(178, 218, 2, 8); ctx.fillRect(174, 222, 12, 2);
  ctx.fillRect(174, 258, 12, 2);
  ctx.strokeStyle = PALETTE.domEdge;
  ctx.lineWidth = 2;
  ctx.strokeRect(160, 200, 40, 80);
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.font = '10px monospace';
    ctx.fillText(LEVELS[i].W + '×' + LEVELS[i].H, r.x + 8, r.y + 36);
    ctx.textAlign = 'right';
...
function paintPlay() {
  ...
  const conflicts = findViolations(puzzle.W, puzzle.H, puzzle.dominoes, puzzle.rc, marks);
  drawScene(ctx, puzzle, marks, conflicts, selected);
