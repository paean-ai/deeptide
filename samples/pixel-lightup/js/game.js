// Pixel Light Up - puzzle play, bulb placement, win detection, save.
...
const SAVE_KEY = 'pixel-lightup-save';
...
let state = null;   // { index, pz, geom, bulbs, ev, won }

function buildLevel(index) {
  const pz = buildPuzzle(LAYOUTS[index]);
  state = {
    index, pz,
    geom: boardGeom(pz),
    bulbs: new Set(), won: false,
  };
  reEval();
  updateHud();
}
...
function reEval() {
  state.ev = evaluate(state.pz, state.bulbs);
}

function toggleBulb(r, c) {
  if (state.won || state.pz.wall[r][c]) return;
  const key = r + ',' + c;
  if (state.bulbs.has(key)) state.bulbs.delete(key);
  else state.bulbs.add(key);
  reEval();
  updateHud();
...
function render() {
  drawBackground(ctx);
  if (state) drawBoard(ctx, state.pz, state.geom, state.ev, state.bulbs);
}
function loop() {
  render();
...
function updateHud() {
  ...
  let dark = 0;
  for (const [r, c] of state.pz.whiteCells) if (!state.ev.lit[r + ',' + c]) dark++;
  document.getElementById('hud-dark').textContent = t('dark') + ' ' + dark;
}
...
canvas.addEventListener('pointerdown', e => {
  ...
  if (r >= 0 && c >= 0 && r < state.pz.h && c < state.pz.w) toggleBulb(r, c);
});
