// Pixel KenKen - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-kenken:save';
...
function startLevel(i) {
  ...
  if (!puzzle) return;
  marks = new Array(puzzle.n).fill(0).map(() => new Array(puzzle.n).fill(0));
  selected = null;
...
function placeDigit(v) {
  ...
  const [x, y] = selected;
  if (v === 0)            marks[y][x] = 0;
  else if (v <= puzzle.n) marks[y][x] = v;
  if (isSolved(puzzle.n, puzzle.cages, marks)) {
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (screen === 'play') {
    ...
    for (const h of padHits) {
      if (hit(p, h)) {
        if (h.kind === 'digit') placeDigit(h.v);
        else if (h.kind === 'erase') placeDigit(0);
        return;
      }
    }
    for (let y = 0; y < puzzle.n; y++) for (let x = 0; x < puzzle.n; x++) {
      const r = cellRect(puzzle.n, x, y);
      if (hit(p, r)) { selected = [x, y]; paint(); return; }
    }
  }
});
...
function paintTitle() {
  ...
  // Demo: 3x3 cell with cage label.
  const dx = 140, dy = 170, dc = 28;
  for (let y = 0; y < 3; y++) for (let x = 0; x < 3; x++) {
    ctx.fillStyle = PALETTE.cell;
    ctx.fillRect(dx + x * dc, dy + y * dc, dc, dc);
  }
  ctx.strokeStyle = PALETTE.cageEdge;
  ctx.lineWidth = 2;
  ctx.strokeRect(dx, dy, dc * 2, dc);     // top row pair
  ctx.fillStyle = PALETTE.cageLabel;
  ctx.font = 'bold 9px monospace';
  ctx.textAlign = 'left';
  ctx.fillText('5+', dx + 2, dy + 8);
...
function paintPlay() {
  ...
  const conflicts = findViolations(puzzle.n, puzzle.cages, marks);
  drawScene(ctx, puzzle, marks, selected, conflicts);
  drawNumberPad(ctx, puzzle.n, lang, padHits);
  drawButton(t(lang, 'backToMenu'), backRect(), true);
}
