// Pixel Kakurasu - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-kakurasu:save';
...
function cycleCell(x, y) {
  // UNDECIDED -> SHADED -> EMPTY -> UNDECIDED
  const v = marks[y][x];
  if (v === UNDECIDED)    marks[y][x] = SHADED;
  else if (v === SHADED)  marks[y][x] = EMPTY;
  else                    marks[y][x] = UNDECIDED;
  if (isSolved(puzzle.n, puzzle.rowT, puzzle.colT, marks)) {
...
function paintTitle() {
  ...
  // Demo: small row with indices + target.
  const dx = 140, dy = 200, dc = 26;
  for (let i = 0; i < 4; i++) {
    ctx.fillStyle = PALETTE.index;
    ctx.font = '9px monospace'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(String(i + 1), dx + i * dc + dc / 2, dy - 8);
    ctx.fillStyle = (i === 0 || i === 3) ? PALETTE.cellShade : PALETTE.cellEmpty;
    ctx.fillRect(dx + i * dc, dy, dc - 1, dc - 1);
  }
  ctx.fillStyle = PALETTE.rowT;
  ctx.font = 'bold 12px monospace';
  ctx.fillText('5', dx + 4 * dc + 14, dy + dc / 2);
  ...
  ctx.fillText(t(lang, 'rulesTxt1'), CANVAS_W / 2, 318);
  ctx.fillText(t(lang, 'rulesTxt2'), CANVAS_W / 2, 334);
  ctx.fillText(t(lang, 'rulesTxt3'), CANVAS_W / 2, 350);
...
function paintPlay() {
  ...
  const conflicts = findViolations(puzzle.n, puzzle.rowT, puzzle.colT, marks);
