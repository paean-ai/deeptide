// Pixel Norinori - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-norinori:save';
...
function startLevel(i) {
  ...
  if (!puzzle) return;
  marks = puzzle.hints.slice();          // hint cells start as their hint value
...
function cycleCell(i) {
  ...
  // UNKNOWN -> SHADED -> UNSHADED -> UNKNOWN
  const v = marks[i];
  if (v === UNKNOWN)        marks[i] = SHADED;
  else if (v === SHADED)    marks[i] = UNSHADED;
  else                      marks[i] = UNKNOWN;
  if (isSolved(puzzle.n, puzzle.regions, marks)) {
...
function paintTitle() {
  ...
  // Demo: small region + one domino.
  const dx = 140, dy = 170, dc = 28;
  ctx.fillStyle = REGION_TINTS[0];
  for (let y = 0; y < 3; y++) for (let x = 0; x < 3; x++) ctx.fillRect(dx + x * dc, dy + y * dc, dc - 1, dc - 1);
  ctx.fillStyle = PALETTE.shaded;
  ctx.fillRect(dx + 1 * dc + 2, dy + 1 * dc + 2, dc - 4, dc - 4);
  ctx.fillRect(dx + 2 * dc + 2, dy + 1 * dc + 2, dc - 4, dc - 4);
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.font = '10px monospace';
    ctx.fillText(LEVELS[i].n + '×' + LEVELS[i].n + ' · ' + LEVELS[i].k + ' regions', r.x + 8, r.y + 36);
    ctx.textAlign = 'right';
...
function paintPlay() {
  ...
  const conflicts = findViolations(puzzle.n, puzzle.regions, marks);
  drawGrid(ctx, puzzle.n, puzzle.regions, puzzle.hints, marks, conflicts);
