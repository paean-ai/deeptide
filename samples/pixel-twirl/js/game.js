// Pixel Twirl - screen flow, tap-to-rotate input, save.

const canvas = document.getElementById('view');
const ctx = canvas.getContext('2d');
ctx.imageSmoothingEnabled = false;

const SAVE_KEY = 'pixel-twirl:save';
const G = {
  screen: 'title',          // title | select | play
  play: null,
  levelIndex: 0,
  dir: 1,                   // current twirl direction
  save: { cleared: [], stars: [] },
};
...
function onTap(p) {
  if (G.screen === 'title') {
    ...
    if (hitR(SPIN_BTN, p)) { G.dir = -G.dir; return; }
    // a board tap -> the nearest 2x2 junction
    const g = gridGeom(s.cols, s.rows);
    if (p.x >= g.ox && p.x <= g.ox + g.bw && p.y >= g.oy && p.y <= g.oy + g.bh) {
      const jc = clamp(Math.round((p.x - g.ox) / g.cell) - 1, 0, s.cols - 2);
      const jr = clamp(Math.round((p.y - g.oy) / g.cell) - 1, 0, s.rows - 2);
      if (doTwirl(s, jr, jc, G.dir)) afterTwirl();
    }
  }
}
...
window.addEventListener('keydown', (e) => {
  if (G.screen === 'play' && G.play && !G.play.over) {
    ...
    else if (e.key === ' ') { G.dir = -G.dir; e.preventDefault(); }
