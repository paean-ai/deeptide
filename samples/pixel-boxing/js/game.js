// Pixel Boxing - screen flow, button + keyboard input, RAF, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-boxing:save';
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (screen === 'play') {
    ...
    for (const key of Object.keys(rs)) {
      if (hit(p, rs[key])) {
        // punch button maps to a generic punch (either glove).
        input(state, key === 'punch' ? 'punchR' : key);
        paint();
        return;
      }
    }
...
window.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A')      input(state, 'dodgeL');
  else if (e.key === 'ArrowRight' || e.key === 'd' || e.key === 'D') input(state, 'dodgeR');
  else if (e.key === 'ArrowDown' || e.key === 's' || e.key === 'S')  input(state, 'block');
  else if (e.key === ' ' || e.key === 'ArrowUp' || e.key === 'w' || e.key === 'W') input(state, 'punchR');
  paint();
});
...
function paintTitle() {
  ...
  // Inline demo: a foe winding up + the player below.
  const demo = buildGame(0);
  demo.foe.state = 'windup'; demo.foe.tell = 'R';
  drawFoe(ctx, demo);
  drawPlayer(ctx, demo);
  ...
  ctx.fillText(t(lang, 'rulesTxt2'), CANVAS_W / 2, 442);
  ctx.fillText(t(lang, 'rulesTxt3'), CANVAS_W / 2, 458);
}
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillStyle = can ? '#28203a' : '#181020';
    ...
    ctx.fillText('HP ' + LEVELS[i].hp + ' · windup ' + LEVELS[i].windup.toFixed(2) + 's', r.x + 8, r.y + 34);
    ctx.textAlign = 'right';
    if (cleared) {
      ctx.fillStyle = '#e85a3a';
...
function paintResult() {
  ...
  ctx.textAlign = 'center';
  ctx.fillText(state.won ? t(lang, 'win') : t(lang, 'lose'), CANVAS_W / 2, 180);
  ctx.fillStyle = '#f8f5e8';
  ctx.font = '14px monospace';
  ctx.fillText(t(lang, 'score') + ': ' + finalScore(state), CANVAS_W / 2, 218);
  ctx.fillStyle = '#a0a8b8';
  ctx.font = '11px monospace';
  ctx.fillText(t(lang, 'high') + ': ' + (save.best[String(state.levelIndex)] || 0), CANVAS_W / 2, 242);
...
function titleStartRect() { return rect(110, 372, 140, 38); }
function langRect()       { return rect(285, 8,   65,  20); }
function backRect()       { return rect(8,   CANVAS_H - 16, 56, 14); }
...
function drawButton(label, r, dim) {
  ...
  ctx.fillRect(r.x, r.y, r.w, 2);
  ctx.fillStyle = dim ? '#f8f5e8' : '#1a1020';
  ctx.font = 'bold 12px monospace';
  ctx.textAlign = 'center';
