// Pixel Archery - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-archery:save';
...
function loop(now) {
  ...
  if (screen === 'play' && state) {
    tick(state, dt);
    if (state.over) {
      // Round cleared if score > some threshold; otherwise just round-done.
      const target = state.cfg.r >= 28 ? 35 : (state.cfg.r >= 24 ? 30 : 25);
      result = {
        won: state.score >= target,
        score: state.score,
        levelIndex: state.levelIndex,
      };
      // Always allow advancing — clear gates by "any score" so you can
      // progress on first try even if you missed.
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (screen === 'result') {
    ...
    if (result.levelIndex + 1 < LEVEL_COUNT && hit(p, nextRect())) {
...
canvas.addEventListener('pointerup', (e) => {
  ...
  pointerId = null;
  releaseAim(state);
  if (!state.arrow) paint();
});
canvas.addEventListener('pointercancel', () => { pointerId = null; if (state) state.aim = null; paint(); });
...
function paintTitle() {
  ...
  // Demo target.
  ctx.fillStyle = PALETTE.target0; ctx.beginPath(); ctx.arc(180, 230, 28, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = PALETTE.target1; ctx.beginPath(); ctx.arc(180, 230, 21, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = PALETTE.target2; ctx.beginPath(); ctx.arc(180, 230, 13, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = PALETTE.target3; ctx.beginPath(); ctx.arc(180, 230, 6, 0, Math.PI*2); ctx.fill();
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillStyle = can ? '#dde6ff' : '#9aa6cc';
    ctx.fillRect(r.x, r.y, r.w, r.h);
    ctx.fillStyle = cleared ? PALETTE.ok : (can ? '#3c4576' : '#7a8099');
    ctx.fillRect(r.x, r.y, r.w, 2);
    ctx.fillStyle = can ? '#0c1230' : '#5a6188';
    ctx.font = 'bold 12px monospace';
    ctx.textAlign = 'left';
    ctx.fillText('L' + (i + 1) + ' ' + LEVELS[i].name[lang === 'zh' ? 1 : 0], r.x + 8, r.y + 20);
    ctx.fillStyle = can ? '#3c4576' : '#5a6188';
    ctx.font = '10px monospace';
    ctx.fillText(t(lang, 'wind') + ': ' + LEVELS[i].wind + ' · r=' + LEVELS[i].r, r.x + 8, r.y + 36);
    ctx.textAlign = 'right';
...
function paintPlay() {
  drawScene(ctx, state, CANVAS_W, CANVAS_H, lang);
  drawButton(t(lang, 'backToMenu'), backRect(), true);
}
...
function paintResult() {
  ...
  ctx.fillText(t(lang, 'score') + ': ' + result.score, CANVAS_W / 2, 210);
  drawButton(t(lang, 'retry'), retryRect());
  if (result.levelIndex + 1 < LEVEL_COUNT) drawButton(t(lang, 'next'), nextRect());
