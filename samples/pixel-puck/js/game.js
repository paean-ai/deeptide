// Pixel Puck - real-time loop, drag input, screens, save.
...
const SAVE_KEY = 'pixel-puck-save';
...
function startLevel(index) {
  ...
  resultShown = false;
  dragXY = { x: state.player.x, y: state.player.y };
  updateHud();
}
...
canvas.addEventListener('pointermove', e => {
  if (!state) return;
  if (document.getElementById('screen-game').classList.contains('hidden')) return;
  if (e.buttons || (e.pressure && e.pressure > 0)) dragXY = localXY(e);
});
