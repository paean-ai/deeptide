// Pixel Helix - screen flow, drag-to-spin + keyboard input, RAF, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-helix:save';
...
canvas.addEventListener('pointermove', (e) => {
  if (!dragPrev || screen !== 'play') return;
  const p = relPoint(e);
  const dx = p.x - dragPrev.x;
  // Each pixel of horizontal drag maps to a small disc rotation; 80 px == 1 segment.
  if (Math.abs(dx) >= 1) {
    rotate(state, dx * (SEG_ANGLE / 80));
    dragPrev = p;
  }
});
canvas.addEventListener('pointerup', () => { dragPrev = null; });
canvas.addEventListener('pointercancel', () => { dragPrev = null; });

window.addEventListener('keydown', (e) => {
  if (screen !== 'play' || !state) return;
  if (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A')      rotate(state, -SEG_ANGLE / 4);
  else if (e.key === 'ArrowRight' || e.key === 'd' || e.key === 'D') rotate(state,  SEG_ANGLE / 4);
});
...
function paintTitle() {
  ...
  // Inline demo: render the L1 first disc.
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillStyle = can ? '#1a1530' : '#100c20';
    ...
    ctx.fillText('depth ' + LEVELS[i].depth + ' · ' + LEVELS[i].bounceHz.toFixed(1) + 'Hz', r.x + 8, r.y + 34);
    ctx.textAlign = 'right';
    if (cleared) {
      ctx.fillStyle = '#bda6ff';
...
function paintResult() {
  ...
  ctx.fillStyle = state.won ? '#bda6ff' : '#ff7a7a';
  ctx.font = 'bold 22px monospace';
  ctx.textAlign = 'center';
...
function drawButton(label, r, dim) {
  ctx.fillStyle = dim ? '#28315c' : '#bda6ff';
  ctx.fillRect(r.x, r.y, r.w, r.h);
  ctx.fillStyle = dim ? '#3c4576' : '#e3d3ff';
  ctx.fillRect(r.x, r.y, r.w, 2);
  ctx.fillStyle = dim ? '#f8f5e8' : '#0d0a18';
  ctx.font = 'bold 12px monospace';
  ctx.textAlign = 'center';
