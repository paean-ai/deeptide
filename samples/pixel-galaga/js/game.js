// Pixel Galaga - screen flow, drag-to-steer + keyboard input, RAF, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-galaga:save';
...
function loop(now) {
  ...
  lastT = now;
  if (dt > 0.05) dt = 0.05;
  // 240Hz substep for stable bullet collisions.
  const sub = 1 / 240;
  let remain = dt;
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (screen === 'play') {
    ...
    dragging = true;
    setPlayerX(state, p.x);
  }
});
canvas.addEventListener('pointermove', (e) => {
  if (!dragging || screen !== 'play') return;
  setPlayerX(state, relPoint(e).x);
});
...
window.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A')      setPlayerX(state, state.player.x - 20);
  else if (e.key === 'ArrowRight' || e.key === 'd' || e.key === 'D') setPlayerX(state, state.player.x + 20);
});
...
function paintTitle() {
  drawBackdrop(ctx, performance.now() / 1000);
  ...
  // Inline demo: a couple of enemies + the player.
  drawEnemy(ctx, 120, 200, '#ff7a7a', false);
  drawEnemy(ctx, 180, 200, '#ffd34a', false);
  drawEnemy(ctx, 240, 200, '#5fc06e', true);
  drawPlayer(ctx, { x: 180, alive: true, hitFlash: 0 });
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillText(LEVELS[i].rows + '×' + LEVELS[i].cols + ' · dive ' + LEVELS[i].diveCd.toFixed(1) + 's', r.x + 8, r.y + 34);
    ctx.textAlign = 'right';
    if (cleared) {
      ctx.fillStyle = '#5fc0ff';
...
function paintPlay() {
  drawBackdrop(ctx, performance.now() / 1000);
  drawEnemies(ctx, state.enemies, state.elapsed);
...
function drawButton(label, r, dim) {
  ...
  ctx.fillRect(r.x, r.y, r.w, 2);
  ctx.fillStyle = dim ? '#f8f5e8' : '#06061a';
  ctx.font = 'bold 12px monospace';
  ctx.textAlign = 'center';
