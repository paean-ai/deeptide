// Pixel Aquarium - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-aquarium:save';
...
function cycleCell(i) {
  ...
  // blank -> water -> air -> blank
  const v = marks[i];
  if (v === UNKNOWN)    marks[i] = WATER;
  else if (v === WATER) marks[i] = AIR;
  else                  marks[i] = UNKNOWN;
  if (isSolved(puzzle.n, puzzle.regions, puzzle.rc, puzzle.cc, marks)) {
...
function paintTitle() {
  ...
  // Demo: tiny tank with water level.
  ctx.fillStyle = PALETTE.glassEdge;
  ctx.fillRect(140, 170, 80, 80);
  ctx.fillStyle = PALETTE.air;
  ctx.fillRect(142, 172, 76, 30);
  ctx.fillStyle = PALETTE.water;
  ctx.fillRect(142, 202, 76, 46);
  ctx.fillStyle = PALETTE.waterCrest;
  ctx.fillRect(142, 202, 76, 1);
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillStyle = can ? '#1c2245' : '#10162a';
    ...
    ctx.font = '10px monospace';
    ctx.fillText(LEVELS[i].n + '×' + LEVELS[i].n + ' · ' + LEVELS[i].k + ' tanks', r.x + 8, r.y + 36);
    ctx.textAlign = 'right';
...
function paintPlay() {
  ...
  const conflicts = findViolations(puzzle.n, puzzle.regions, puzzle.rc, puzzle.cc, marks);
