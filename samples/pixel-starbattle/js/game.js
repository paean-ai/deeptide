// Pixel Star Battle - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-starbattle:save';
...
function cycleCell(i) {
  ...
  marks[i] = (marks[i] + 1) % 3;       // blank -> star -> X -> blank
  if (isSolved(puzzle.n, puzzle.k, puzzle.regions, marks)) {
...
function paintTitle() {
  ...
  for (let i = 0; i < 50; i++) {
    ctx.fillRect((i * 41 + 7) % CANVAS_W, (i * 73 + 17) % CANVAS_H, 1, 1);
  }
  ...
  // Demo: a tiny 3x3 grid with one star.
  const dx = 140, dy = 170, dc = 26;
  for (let y = 0; y < 3; y++) for (let x = 0; x < 3; x++) {
    ctx.fillStyle = REGION_TINTS[(x + y) % REGION_TINTS.length];
    ctx.fillRect(dx + x * dc, dy + y * dc, dc - 1, dc - 1);
  }
  drawStar(ctx, dx + dc + dc / 2, dy + dc + dc / 2, dc);
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.font = '10px monospace';
    ctx.fillText(`${LEVELS[i].n}×${LEVELS[i].n} · k=${LEVELS[i].k}`, r.x + 8, r.y + 36);
    ctx.textAlign = 'right';
...
function paintPlay() {
  ctx.fillStyle = PALETTE.bg;
  ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);
  drawHud(ctx, lang, puzzle.levelIndex, puzzle.cfg || LEVELS[puzzle.levelIndex], marks, elapsed);
  const conflicts = findViolations(puzzle.n, puzzle.k, puzzle.regions, marks);
  drawGrid(ctx, puzzle.n, puzzle.regions, marks, conflicts);
