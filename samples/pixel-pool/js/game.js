// Pixel Pool - screen flow, input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-pool:save';
  ...
  if (screen === 'play' && state) {
    ...
    if (state.over) {
      result = {
        ...
        score: gameScore(state), strokes: state.strokes, fouls: state.fouls,
      };
      if (result.won) {
...
canvas.addEventListener('pointerup', (e) => {
  ...
  pointerId = null;
  releaseAim(state);
  if (state && state.state !== 'rolling') paint();
});
...
function paintTitle() {
  ...
  // Demo: table felt + a cue + a coloured ball.
  ctx.fillStyle = PALETTE.felt;
  ctx.fillRect(110, 180, 140, 80);
  ctx.fillStyle = PALETTE.cue;
  ctx.beginPath(); ctx.arc(140, 220, 7, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#e8554f';
  ctx.beginPath(); ctx.arc(220, 220, 7, 0, Math.PI*2); ctx.fill();
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.font = '10px monospace';
    ctx.fillText(LEVELS[i].balls.length + ' balls · ' + LEVELS[i].strokes + ' strokes', r.x + 8, r.y + 36);
    ctx.textAlign = 'right';
...
function paintPlay() {
  ...
  if (!state.started && !state.aim) {
    ctx.fillStyle = 'rgba(247, 230, 154, 0.85)';
    ctx.font = 'bold 12px monospace';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(t(lang, 'aim'), CANVAS_W / 2, 430);
...
function paintResult() {
  ...
  ctx.fillText(t(lang, 'strokes') + ': ' + result.strokes + '/' + state.cfg.strokes + '  ' +
               t(lang, 'fouls') + ': ' + result.fouls, CANVAS_W / 2, 220);
