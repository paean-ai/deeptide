// Pixel Rally - real-time loop, drag input, screens, save.
...
const SAVE_KEY = 'pixel-rally-save';
...
function loop(now) {
  ...
  if (playing) {
    tick(state, dt, dragX != null ? dragX : state.playerX);
    updateHud();
    if (state.over) showResult();
  }
  if (state) drawAll(ctx, state);
...
function updateHud() {
  ...
  document.getElementById('hud-score').textContent = state.playerScore + ' - ' + state.cpuScore;
}
...
function showResult() {
  ...
  setTimeout(() => {
    ...
    document.getElementById('res-line').textContent =
      t('winLine', state.playerScore, state.cpuScore);
...
// ---- input: drag anywhere sets the player paddle x ----------------------
function localX(e) {
...
canvas.addEventListener('pointermove', e => {
  if (!state) return;
  if (document.getElementById('screen-game').classList.contains('hidden')) return;
  if (e.buttons || (e.pressure && e.pressure > 0)) dragX = localX(e);
});
