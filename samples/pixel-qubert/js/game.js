// Pixel Qubert - screen flow, tap-quadrant + keyboard input, RAF, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-qubert:save';
...
function loop(now) {
  ...
  lastT = now;
  if (dt > 0.05) dt = 0.05;
  if (state && !state.player.alive && !state.over) {
    state.player.respawn = (state.player.respawn || 0) - dt;
    if (state.player.respawn <= 0) resetPlayer(state);
  }
  tick(state, dt);
  if (state.over) {
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (screen === 'play') {
    ...
    // Tap quadrant relative to the player's cube screen position.
    const here = standCenter(state.player.r, state.player.c);
    const dir = dirFromTap(p.x - here.x, p.y - here.y);
    if (dir) hop(state, dir);
  }
});

window.addEventListener('keydown', (e) => {
  if (screen !== 'play' || !state) return;
  if (e.key === 'q' || e.key === 'Q' || e.key === '7')      hop(state, 'nw');
  else if (e.key === 'e' || e.key === 'E' || e.key === '9') hop(state, 'ne');
  else if (e.key === 'z' || e.key === 'Z' || e.key === '1') hop(state, 'sw');
  else if (e.key === 'c' || e.key === 'C' || e.key === '3') hop(state, 'se');
  // Arrow keys map up = ne, left = nw, right = se, down = sw as a fallback
  else if (e.key === 'ArrowUp')    hop(state, 'ne');
  else if (e.key === 'ArrowLeft')  hop(state, 'nw');
  else if (e.key === 'ArrowRight') hop(state, 'se');
  else if (e.key === 'ArrowDown')  hop(state, 'sw');
});
...
function paintTitle() {
  ...
  // Render a small demo pyramid (2 rows = 3 cubes) + player on top.
  const demo = {
    levelIndex: 0,
    cfg: LEVELS[0],
    cubes: [[1], [0, 1]],
    ...
    enemies: [],
    flash: 0, completed: 0, score: 0, lives: 2, elapsed: 0, over: false, won: false,
  };
  // Temporarily relocate the pyramid origin for the demo by offsetting.
  ctx.save();
  ctx.translate(0, 60);
  drawPyramid(ctx, demo);
  drawPlayer(ctx, demo);
  ctx.restore();
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillStyle = can ? '#1c1838' : '#100c20';
    ctx.fillRect(r.x, r.y, r.w, r.h);
    ctx.fillStyle = cleared ? '#5fc06e' : (can ? '#ffaa40' : '#5a6188');
    ...
    ctx.fillText(LEVELS[i].stages.length + '-hop · enemy ' + LEVELS[i].enemyCd.toFixed(1) + 's', r.x + 8, r.y + 34);
    ctx.textAlign = 'right';
    if (cleared) {
      ctx.fillStyle = '#ffaa40';
...
function paintResult() {
  ...
  ctx.fillStyle = state.won ? '#ffaa40' : '#ff7a7a';
  ctx.font = 'bold 22px monospace';
  ctx.textAlign = 'center';
...
function drawButton(label, r, dim) {
  ctx.fillStyle = dim ? '#28315c' : '#ffaa40';
  ctx.fillRect(r.x, r.y, r.w, r.h);
  ctx.fillStyle = dim ? '#3c4576' : '#ffd078';
  ctx.fillRect(r.x, r.y, r.w, 2);
  ctx.fillStyle = dim ? '#f8f5e8' : '#0a0a1c';
  ctx.font = 'bold 12px monospace';
  ctx.textAlign = 'center';
