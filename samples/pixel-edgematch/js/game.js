// Pixel Edgematch - screen flow, tap input, save.

const CANVAS_W = 360, CANVAS_H = 480;
const SAVE_KEY = 'pixel-edgematch:save';
...
function onCellTap(cell) {
  if (state.selected < 0) {
    state.selected = cell;
  } else if (state.selected === cell) {
    rotateCell(state, cell);                 // tap the held tile -> rotate
  } else {
    swapCells(state, state.selected, cell);  // tap another -> swap
    state.selected = -1;
  }
  if (isSolved(state)) {
    state.over = true;
    ...
    if (save.best[key] === undefined || save.best[key] > state.moves) save.best[key] = state.moves;
    persist();
    screen = 'result';
  }
  paint();
...
function paintTitle() {
  ...
  ctx.fillText(t(lang, 'title'), CANVAS_W / 2, 102);
...
function paintLevels() {
  ...
  for (let i = 0; i < LEVEL_COUNT; i++) {
    ...
    ctx.fillStyle = can ? PALETTE.panel : '#1c1e28';
    ...
    ctx.font = '10px monospace';
    ctx.fillStyle = can ? '#aab0c4' : '#5b6072';
...
function paintPlay() {
  ...
  ctx.textAlign = 'center';
  ctx.textBaseline = 'alphabetic';
  ctx.fillText(t(lang, state.selected >= 0 ? 'rulesTxt2' : 'rulesTxt1'), CANVAS_W / 2, 452);
  drawButton(t(lang, 'backToMenu'), backRect(), true);
}
...
function drawButton(label, r, dim) {
  ctx.fillStyle = dim ? '#2a2d3c' : '#f4c44a';
  ctx.fillRect(r.x, r.y, r.w, r.h);
  ctx.fillStyle = dim ? '#3a3e50' : '#ffe08a';
  ctx.fillRect(r.x, r.y, r.w, 2);
  ctx.fillStyle = dim ? '#f3f1e6' : '#1b1d28';
  ctx.font = 'bold 12px monospace';
  ctx.textAlign = 'center';
