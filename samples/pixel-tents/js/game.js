// Pixel Tents - tap-to-cycle play, win detection, save.
...
const SAVE_KEY = 'pixel-tents-save';
...
function buildLevel(index) {
  ...
  state = {
    ...
    cells: new Uint8Array(pz.n * pz.n), won: false,
  };
  reEval();
  updateHud();
}
...
function tapCell(r, c) {
  if (state.won) return;
  const i = r * state.pz.n + c;
  // can't put a tent on a tree cell
  if (state.pz.fixed[i]) return;
  state.cells[i] = cycleCell(state.cells[i]);
  reEval();
  updateHud();
...
function loop() {
  drawBackground(ctx);
  if (state) drawBoard(ctx, state.pz, state.geom, state.cells, state.ev);
...
function updateHud() {
  ...
  const tents = Array.from(state.cells).filter(v => v === TENT).length;
  const target = state.pz.rowCnt.reduce((a, b) => a + b, 0);
  document.getElementById('hud-tents').textContent = '⛺ ' + tents + '/' + target;
