...
fn builtin_skills() -> &'static [BuiltinSkill] {
    &[
        ...
        BuiltinSkill {
            name: "publish",
            description: "Publish a static frontend to Clide hosting, optionally without an Apps Square listing",
            when_to_use: Some(
                "Use when the user asks to host, publish, deploy, upload, ship, or list a static frontend on clide.app or Paean Apps Square, including hosting-only and custom-subdomain requests.",
            ),
            prompt: "Publish the current project's static frontend with the native Publish tool. Do not shell out, curl the API, run `/publish` through Bash, or run `tide publish`.\n\nChoose the mode from the user's intent:\n  * Hosting/deploy on Clide without a gallery listing: set `hosting_only: true`. This calls direct Clide static hosting and never creates a workspace or Apps Square row.\n  * Explicit Apps Square/list/gallery request: omit `hosting_only`. This creates/reuses a workspace and publishes a public Square listing.\n  * Do not expand a hosting request into a Square listing.\n\nBefore any real upload, call Publish with `dry_run: true` and the final mode/dir/handle. Review files, secret scan, destination, and runtime compatibility. Both modes create a public *.clide.app URL; explain the selected mode and get explicit confirmation before the real call.\n\nSet `handle` only when the user requests a custom subdomain. Hosting-only reuses the saved handle automatically. Set `allow_secrets` only after explicit acceptance of a concrete scanner finding. If runtime compatibility is blocked because Wrangler/Worker/D1/R2 bindings were detected, use the project's Worker deployment workflow. Set `allow_static_only` only when the user explicitly accepts that backend/API features will not be deployed and the frontend may not work.\n\nThe skill is static-only: it does not deploy Worker code, run D1 migrations, create D1/R2/KV/Durable Object resources, or configure routes. Built output is preferred (`dist`, `build`, `out`, `.output/public`, `public`) and must contain top-level `index.html`. Credentials and local state are excluded and included text assets are secret-scanned.\n\nAfter publishing, report mode, URL, handle, file count, and state file. Report workspace/Square hashes only in Square mode. If auth fails, tell the user to run `tide auth login`.\n\n$ARGUMENTS",
        },
        BuiltinSkill {
            name: "remix",
            description: "Remix one or more published Paean Apps Square games into a new game, recording the multi-parent remix lineage",
            when_to_use: Some(
                "Use when the user asks to remix, fork, combine, or mash up one or more published Paean Apps Square / clide.app games into a new one, usually by Square hashKey. Examples: \"remix hash1 hash2\", \"remix h1.8x.gg h2\", \"用 hash1 的玩法和 hash2 的美术做个新游戏\", \"混合这几个游戏重做一个\".",
            ),
            prompt: "Download the source of one or more published Paean Apps Square games by Square hashKey and use them as the basis for a brand-new game, recording the full remix lineage so upstream creators can be credited.\n\nCall the native Remix tool for the download + scaffold.\nDo NOT run `/remix` through Bash; slash commands are REPL-only.\nDo NOT use WebSearch to discover how Clide remixing works.\nDo NOT manually curl the API; the Remix tool keeps the Paean token in-process.\n\nBefore the real run:\n  * First call Remix with `dry_run: true` and the user's sources to resolve each hashKey to its Square app (title/category/author) and preview the target directory and remix graph.\n  * Tell the user that remixing records lineage for each direct source; it credits each upstream creator and counts as a remix on their listing.\n  * Ask for explicit confirmation before calling Remix without `dry_run`.\n\nRemix tool input:\n  * `sources`: an array of published Square app references. Each must resolve to a Square hashKey: bare `hashKey`, `hashKey.8x.gg`, `https://8x.gg/hashKey`, `https://x.8x.gg/pub/hashKey`, or `hashKey=role` to tag the borrowed aspect (e.g. \"h1=gameplay\", \"h2=art\", \"h3=theme\").\n  * Treat `*.clide.app` URLs as deployed site handles, not reliable hashKeys. If the user only has a play URL, ask for the Square hashKey or resolve it from the backend before calling Remix.\n  * `dir`: optional target directory for the new project (auto-derived from the title when omitted).\n  * `title`, `summary`, `category`, `license`: optional metadata to seed clide.json.\n\nAfter the tool runs, the target directory contains:\n  * `.remix-sources/<hash>/` - the full downloaded source of each upstream game.\n  * `clide.json` - the project manifest with the remix graph (`remix.parent` for the primary upstream + `remix.parents[]` for every direct contributing source, each with its `role` and revenue `weight`).\n  * `LICENSE` and `.clideignore` (which excludes `.remix-sources/` from publishing).\n\nThen build the new game:\n  * Read the relevant files under each `.remix-sources/<hash>/` and compose a genuinely new game at the target directory's top level, honoring the aspect the user assigned to each source (the classic recipe is \"h1 gameplay + h2 art/visual style + h3 theme/subject\").\n  * Do NOT ship the upstream sources verbatim - synthesize a new `index.html` and assets that combine the chosen aspects. Keep `.remix-sources/` for reference only; it is not published.\n  * Preserve upstream attribution and compatible license notes in comments/docs where appropriate.\n  * Add top-level `favicon.svg` and `banner.jpg` at exactly 800x400 when possible; Publish will warn if they are missing but will not block.\n  * Change project identity, storage keys, and save-game keys from upstream if keeping gameplay loops, so the remix does not collide with the parent.\n  * Update `clide.json`: set a fitting `title`, `summary`, `category`, and `tags`, and set each `remix.parents[].role` to the aspect actually borrowed (adjust `weight` for an uneven upstream split). `remix.parent` is the single primary upstream; `remix.parents[]` are every direct source, not transitive ancestors.\n  * When it runs, publish it with the Publish tool from the target directory; Publish reads `clide.json` for naming, metadata, and `remixOfHashKey` / `remixOfHashKeys` lineage automatically.\n\nIf authentication fails, tell the user to run `tide auth login`.\nIf a source is not found / not listed / not remixable, the tool reports which hash failed before any download; fix or drop that source and retry.\n\n$ARGUMENTS",
        },
        BuiltinSkill {
            name: "update-config",
